...
possibili diritti/agevolazioni da approfondire.

- Se è presente una chiave Gemini (GEMINI_API_KEY), usa l'AI per generare l'analisi.
- Altrimenti funziona in modalità DEMO, con poche regole semplici, così l'app è
  provabile anche senza chiave.

NB: in questa fase l'analisi è puramente INDICATIVA e non costituisce consulenza.
"""
...
class Profilo(BaseModel):
    ...
    isee_indicativo: str = Field(
        "", description="Fascia ISEE indicativa, es. 'meno di 10.000'"
    )
...
def genera_analisi(profilo: Profilo) -> dict:
    ...
    if chiave:
        ...
        except Exception as errore:  # in caso di problemi, non blocchiamo l'utente
...
ITALIANI che l'utente potrebbe approfondire. Non garantire il diritto: indica solo
opportunità plausibili da verificare. Non inventare dati non desumibili dal profilo.
...
{{
  "opportunita": [
    {{
      ...
      "categoria": "Famiglia | Casa | Lavoro | Reddito | Salute | Altro",
...
def _analisi_demo(profilo: Profilo) -> dict:
    ...
    isee_basso = any(
        ...
        for s in ["meno", "0-10", "0 - 10", "sotto", "basso", "10.000", "15.000"]
    )

    if profilo.figli_minori or profilo.numero_figli > 0:
        opportunita.append(
            {
                "titolo": "Assegno Unico e Universale per i figli",
                "categoria": "Famiglia",
                "perche": "Hai indicato la presenza di figli nel nucleo.",
                "cosa_verificare": "Presentazione domanda INPS e ISEE aggiornato.",
                "confidenza": "alta",
            }
        )
    ...
    if isee_basso:
        opportunita.append(
            {
                "titolo": "Bonus sociale per bollette (luce, gas, acqua)",
                "categoria": "Reddito",
                "perche": "Hai indicato un ISEE indicativo basso.",
                "cosa_verificare": "Soglia ISEE prevista per l'anno in corso.",
                "confidenza": "media",
            }
        )
    ...
    if "affitto" in profilo.situazione_abitativa.lower():
        opportunita.append(
            {
                "titolo": "Detrazioni/contributi per l'affitto",
                "categoria": "Casa",
                "perche": "Hai indicato di vivere in affitto.",
                "cosa_verificare": "Tipo di contratto e requisiti di reddito.",
                "confidenza": "media",
            }
        )
    ...
    if "disoccup" in profilo.situazione_lavorativa.lower():
        opportunita.append(
            {
                "titolo": "NASpI (indennità di disoccupazione)",
                "categoria": "Lavoro",
                "perche": "Hai indicato una situazione di disoccupazione.",
                "cosa_verificare": "Requisiti contributivi e termini di domanda.",
                "confidenza": "media",
            }
        )
    ...
    if not opportunita:
        opportunita.append(
            {
                "titolo": "Verifica generale delle agevolazioni ISEE",
                "categoria": "Altro",
                "perche": "Con più informazioni l'analisi diventa più precisa.",
                "cosa_verificare": "Aggiorna il questionario o carica l'ISEE.",
                "confidenza": "bassa",
            }
        )
